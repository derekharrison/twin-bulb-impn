...
Results of the five-component system as well as the results of the scheme based on a dynamic multi-directional search are shown in figure \ref{fig:fig_3}. The solution of the explicit scheme overshoots somewhat for two of the components, but the remaining results agree fairly well. The method based on explicit time discretization required prohibitively large numbers of timesteps.
